...
export const drawHolisticResults = (
    ...
    dataCanvasRef: RefObject<HTMLCanvasElement>, 
    videoElement: HTMLVideoElement) => {
    if (!results || !previewCanvasRef.current || !dataCanvasRef.current || !videoElement) return; // 하나라도 없으면 반환
    ...
    if (!previewCtx || !dataCtx) return; // TS 안전 처리

    [previewCanvas, dataCanvas].forEach(canvas => {
    ...
    if(inputSource){
        try{
            previewCtx.drawImage(inputSource, 0, 0, previewCanvas.width, previewCanvas.height);
        } catch (e){
    ...
    const drawContent = (canvasCtx : CanvasRenderingContext2D) => {
