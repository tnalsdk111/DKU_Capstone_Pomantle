...
const GamePage = () => {
  ...
  const startTimer = (seconds:number) => { // startTimer(seconds)를 주면 setTimer(seconds)를 실행
  ...
  useEffect(() => {
    ...
    const handleMediaPipeResults = (results: any) => {
      // 1. 비디오와 캔버스가 모두 있는지 확인
      const video = webcamRef.current?.video;
      
      if (video && canvasRef.current) {
        // 2. 존재가 확인되었으므로 에러 없이 실행 가능
        // 만약 여전히 캔버스에서 에러가 난다면 'as'를 섞어주세요.
        drawHolisticResults(
          results, 
    ...
    if(webcamRef.current && webcamRef.current.video){ // 웹캠과 웹캠 비디오가 잘 작동되면 
      const videoElement = webcamRef.current?.video as HTMLVideoElement; // videoElement에 비디오를 가져오고
